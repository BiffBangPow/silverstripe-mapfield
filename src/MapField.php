...
class MapField extends FormField
{
    ...
    public function setupChildren()
    {
        ...
        )->addExtraClass('osmmapfield-latfield no-change-track');

        $this->lngField = HiddenField::create(
            $name . '[Longitude]',
            'Lng',
            $this->recordFieldData('Longitude')
        )->addExtraClass('osmmapfield-lngfield no-change-track');

        $this->zoomField = HiddenField::create(
            $name . '[Zoom]',
            'Zoom',
            $this->recordFieldData('Zoom')
        )->addExtraClass('osmmapfield-zoomfield no-change-track');
        ...
        if ($this->options['show_search_box']) {
            $this->children->push(
                TextField::create('Search')
                    ->addExtraClass('osmmapfield-searchfield')
